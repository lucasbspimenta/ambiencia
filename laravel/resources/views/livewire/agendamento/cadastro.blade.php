            ...
            <div class="mb-2 text-sm">
                <div class="form-row">
                    <div class="col-md-4 mb-3 ">
                        ...
                        <input type="text" wire:model.defer="inicio" wire:loading.attr="disabled" onchange="this.dispatchEvent(new InputEvent('input'))" class="form-control input-150 datepicker @error('inicio') is-invalid @enderror" name="inicio" id="inicio" placeholder="dd/mm/aaaa" value="" required>
                        @error('inicio') <div class="invalid-feedback is-invalid">{{ $message }}</div>@enderror
                    </div>
                    <div class="col-md-4 mb-3 ">
                        <label for="final">Final: </label>
                        <input type="text" wire:model.defer="final"  wire:loading.attr="disabled" onchange="this.dispatchEvent(new InputEvent('input'))" class="form-control input-150 datepicker @error('final') is-invalid @enderror" name="final" id="final" value="" placeholder="dd/mm/aaaa">
                        @error('final') <div class="invalid-feedback is-invalid">{{ $message }}</div>@enderror
                    </div>
                    <div class="col-md-4 mb-3 ">
                        <label class="active">Tipo de Agendamento</label>
                        <select class="browser-default custom-select @error('agendamento_tipos_id') is-invalid @enderror" wire:model.debounce.defer="agendamento_tipos_id" wire:loading.attr="disabled" required>
                        ...
                        @error('agendamento_tipos_id') <div class="invalid-feedback is-invalid">{{ $message }}</div>@enderror
                    </div>
                </div>
                <div class="form-row">
                    <div class="col-md-12 mb-3 ">
                        <label class="active">Unidade</label>
                        <select class="browser-default custom-select @error('unidade_id') is-invalid @enderror" wire:model.debounce.defer="unidade_id" wire:loading.attr="disabled" required>
                            <option value="" selected>Selecione a unidade</option>
                            @forelse(\App\Models\Unidade::select('id','codigo','tipoPv','nome')->orderBy('nome', 'ASC')->get() as $unidade)
                                <option value="{{ $unidade->id }}">{{ Str::padLeft($unidade->codigo,4,'0') ?? '' }} - {{ $unidade->tipoPv ?? '' }}&nbsp;{{ $unidade->nome }}</option>
                            @empty
                                <option value="" disabled>Nenhuma unidade encontrada.</option>
                            @endforelse
                        </select>

                        @error('unidade_id') <div class="invalid-feedback is-invalid">{{ $message }}</div>@enderror
                    </div>
                </div>
                <div class="form-row">
                    <div class="col-md-12 mb-3 ">
