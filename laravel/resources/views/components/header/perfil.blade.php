@auth
    @if (Auth::user()->is_admin)
        <li class="nav-item text-white mt-1">
            <div class="my-0 mr-3">
                <select class="form-control bg-white custom-select-sm"
                    onchange="window.location='{{ route('adm.simulausuario') }}/' + this.value">
                    <option value="" selected>Nenhum</option>
                    @foreach (App\Models\User::where('matricula', '!=', Auth::user()->matricula)->orderBy('name')->get()
        as $usuario_simular)
                        <option value="{{ $usuario_simular->matricula }}">{{ $usuario_simular->matricula }} -
                            {{ $usuario_simular->name }}</option>
                    @endforeach
                </select>
            </div>
        </li>
                        ...
                        <tbody>
                            @if (Auth::user()->is_simulado)
                                <tr>
                                    ...
                                    <td class=""><small>{{ Auth::user()->matricula }}</small><br><a
                                            href="{{ route('limpasimulacao') }}"
                                            class="text-danger font-small"><small>Cancelar</small></a></td>
                                </tr>
                            @endif
                            <tr>
                            ...
                            <tr>
                                ...
                                <td class="">
                                    <small>
                                        <a style="padding:0px; font-size:90%; font-weight: 400;padding-top:4px;"
                                            class="text-nowrap" target="_blank"
                                            href="https://teams.microsoft.com/l/chat/0/0?users={{ trim(Auth::user()->equipe->gestor) }}@corp.caixa.gov.br">
